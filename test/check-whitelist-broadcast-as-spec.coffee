http = require 'http'
CheckWhitelistBroadcastAs = require '../'

describe 'CheckWhitelistBroadcastAs', ->
  beforeEach ->
    @whitelistManager =
      checkBroadcastAs: sinon.stub()

    @sut = new CheckWhitelistBroadcastAs
  ...
  describe '->do', ->
    ...
    describe 'when called with a valid job', ->
      beforeEach (done) ->
        @whitelistManager.checkBroadcastAs.yields null, true
        job =
          metadata:
            auth:
              uuid: 'green-blue'
      ...
      it 'should call the whitelistManager checkBroadcastAs with the auth.uuid and the auth.as', ->
        expect(@whitelistManager.checkBroadcastAs).to.have.been.calledWith
          emitter: 'dim-green'
    ...
    describe 'when called with a job that with a device that has an invalid whitelist', ->
      beforeEach (done) ->
        @whitelistManager.checkBroadcastAs.yields null, false
        job =
          metadata:
            auth:
              uuid: 'puke-green'
    ...
    describe 'when called and the checkBroadcastAs yields an error', ->
      beforeEach (done) ->
        @whitelistManager.checkBroadcastAs.yields new Error "black-n-black"
        job =
          metadata:
            auth:
              uuid: 'puke-green'
